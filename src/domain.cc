...
namespace NodeLibvirt {
  ...
  Handle<Value> Domain::Save(const Arguments& args) {
    ...
    if(!args[1]->IsString()) {
      return ThrowException(Exception::TypeError(
            String::New("You must specify domain XML string as the second argument")));
    }
    ...
    if(!args[2]->IsArray()) {
      return ThrowException(Exception::TypeError(
            String::New("You must specify flags array as the third argument")));
    }
    ...
    if (!args[3]->IsFunction()) {
      return ThrowException(Exception::TypeError(
            String::New("You must specifiy a callback function as the fourth argument")));
    }
